...
print( 'Enter choice of position: ')
goalPos = [float(x) for x in input().split()]

goalOri = eul2rot(np.array([0, 90, -90]))

goalPose = toPose(goalOri,np.reshape(goalPos,(3,1)))
print(str(goalPose))

thetas_o = np.zeros((6,1))
...
#FIX thetas is returning as none
thetas = jk.jaco_IK(goalPose)
print(str(thetas))
# Start simulation
...
vrep.simxSetObjectOrientation(clientID, goalFrame,jacoFrame, rot2eul(goalOri), vrep.simx_opmode_blocking)
...
# stuck here on moving to the targ position since thetas is returning as NONE
for i in range(0, 6):
